...
class LeCartCustom(LeBasecart):
    ...
    def get_categories_ext_export(self, categories):
        ...
        categories_ext_queries = {
            ...
            'languages': {
                'type': 'select',
                'query': "SELECT * FROM categories_description cd INNER JOIN languages lg " + " ON cd.language_id = lg.languages_id WHERE cd.categories_id IN " + self.list_to_in_condition(
...
def get_products_ext_export(self, products):
    ...
    product_ext_queries = {
        'product_description': {
            'type': "select",
            'query': "SELECT * FROM  products_description WHERE product_id IN " + product_id_con,
        },
        'products_to_categories': {
            'type': "select",
            'query': "SELECT * FROM  products_to_categories WHERE product_id IN  " + product_id_con,
        },
    }
...
def convert_product_export(self, product, products_ext):
    ...
    product_data['updated_at'] = to_str(datetime.fromtimestamp(to_int(product['products_last_modified']))) if to_int(
    ...
    product_categories = get_list_from_list_by_field(products_ext_data['products_to_categories'], 'product_id',
                                                     product['product_id'])
...
def check_customer_import(self, convert, customer, customers_ext):
    return True if self.get_map_field_by_src(self.TYPE_CUSTOMER, convert['customers_id'], convert['code']) else False
...
def get_orders_main_export(self):
    ...
    query = {
        'type': 'select',
        'query': "SELECT * FROM orders WHERE orders_id > " + to_str(id_src) + " ORDER BY orders_id ASC LIMIT " + to_str(
            limit)
    }
...
def convert_order_export(self, order, orders_ext):
    ...
    order_data['currency'] = ''
    order_data['created_at'] = datetime.fromtimestamp(to_int(get_value_by_key_in_dict(order, 'orderdate', 0))).strftime(
        '%Y-%m-%d %H:%M:%S')
