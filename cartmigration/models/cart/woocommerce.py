...
class LeCartWoocommerce(LeCartWordpress):
	...
	def get_query_display_import_source(self, update = False):
		...
		queries = {
			'categories': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM categories WHERE categories_id " + compare_condition + to_str(self._notice['process']['categories']['id_src']),
			},
			'products': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM products WHERE products_id " + compare_condition + to_str(
					self._notice['process']['products']['id_src']),
			},
			'customers': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM customers WHERE customers_id " + compare_condition + to_str(
					self._notice['process']['customers']['id_src']),
			},
			'orders': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM orders WHERE orders_id " + compare_condition + to_str(
