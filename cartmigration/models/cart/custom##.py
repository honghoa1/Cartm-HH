...
class LeCartCustom(LeBasecart):
	...
	def get_categories_main_export(self):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM categories WHERE categories_id > " + to_str(id_src) + " ORDER BY categories_id ASC LIMIT " + to_str(limit)
		}
	...
	def get_categories_ext_export(self, categories):
		...
		categories_ext_queries = {
			'categories_description':{
				'type': 'select',
				'query': "SELECT * FROM categories_description WHERE categories_id IN " + self.list_to_in_condition(category_ids)
			},
			'languages':{
				'type': 'select',
				'query': "SELECT * FROM languages as lg INNER JOIN categories_description as cd" + " ON lg.languages_id = cd.language_id WHERE cd.categories_id IN " + self.list_to_in_condition(map(int, category_ids))
			},
		}
	...
	def get_orders_main_export(self):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM orders WHERE orders_id > " + to_str(id_src) + " ORDER BY orders_id ASC LIMIT " + to_str(limit)
		}
